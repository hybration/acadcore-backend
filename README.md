...
5. **Bootstrap the first institution + admin** (there's no other way to get a first user into an empty database):
   ```
   DATABASE_URL=<your postgres superuser connection string> node scripts/seed-first-admin.js \
     --institution "University of Ilorin" --slug uni-ilorin \
     --email admin@uniilorin.edu.ng --password "changeme123!" --name "Admin Name"
   ```
   Run this connected as `postgres` (or another role with `BYPASSRLS`) — it must bypass RLS since no tenant context exists yet. From then on, log in as this user and create every other account through `POST /api/users`.
6. `npm run dev`
...
- `scripts/seed-first-admin.js` — one-time bootstrap script, run manually, not an API endpoint (see Setup step 5 for why).
...
- **Cross-institution `super_admin` support** — the backend uses a single DB connection role for everything; the `acadcore_super_admin`/`BYPASSRLS` split described in the Phase 5 SQL comments was never wired into `src/db.js`. A `super_admin` user would currently hit the same RLS as everyone else and see nothing. `institution_admin` (full control within one institution) is what's actually usable today, and is what the seed script creates. Build the dual-role connection switch only once you're onboarding a second institution.
...
- **students import**: `matric_no, first_name, last_name, programme, session` (programme and session must match existing `programmes.name` / `academic_sessions.name` values for that institution — resolved by name, not ID, since that's what a spreadsheet will contain)
